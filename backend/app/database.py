...
def _force_ipv4_in_url(db_url: str) -> str:
    try:
        ...
        infos = socket.getaddrinfo(host, port or 5432, family=socket.AF_INET, type=socket.SOCK_STREAM)
        if not infos:
            return db_url
        ipv4 = infos[0][4][0]
        # Rebuild URL with IPv4 literal
        netloc = f"{parsed.username}:{parsed.password}@{ipv4}:{port}" if parsed.username and parsed.password else f"{ipv4}:{port}"
