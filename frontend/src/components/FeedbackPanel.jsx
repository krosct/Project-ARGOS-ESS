...
export default function FeedbackPanel() {
  ...
  function send() {
    if (!text.trim())
      return alert("Por favor, escreva um comentário antes de enviar.");
    setText("");
    setSent(true);
  ...
  return (
    <div className="mt-4 bg-gray-50 p-4 rounded-xl border border-gray-100">
      ...
      <textarea
        ...
        className="w-full h-20 mt-2 p-2 rounded-lg border border-gray-300 resize-none"
      />
      <div className="flex items-center justify-end gap-2 mt-2">
        <button
          onClick={send}
          className="btn bg-accent text-white px-4 py-2 rounded-xl"
        >
          Enviar
        </button>
      </div>
